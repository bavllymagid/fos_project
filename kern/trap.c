...
void placeAL(struct Env *curenv, uint32 fault_va);
...
void page_fault_handler(struct Env *curenv, uint32 fault_va)
{
	...
	LIST_FOREACH(el, &(curenv->SecondList))
	{
		//if found put the found item in the active list
		if (el->virtual_address == fault_va)
		{
	...
	if (curenv->SecondListSize == LIST_SIZE(&(curenv->SecondList)))
	{
		...
		//to add to second list and put the new one in active list
...
void placeAL(struct Env *curenv, uint32 fault_va)
{
	...
	struct Frame_Info *f_ptr = NULL;
	int ret = allocate_frame(&f_ptr);
	map_frame(curenv->env_page_directory, f_ptr, (void *)fault_va, PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
...
void placeSC(struct Env *curenv, uint32 fault_va)
{
	...
	struct WorkingSetElement *el = LIST_LAST(&(curenv->ActiveList));
	LIST_REMOVE(&(curenv->ActiveList), el);
	LIST_INSERT_HEAD(&(curenv->SecondList), el);
	pt_set_page_permissions(curenv, el->virtual_address, 0, PERM_PRESENT);
	placeAL(curenv, fault_va);
}
